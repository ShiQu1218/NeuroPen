...
<p align="center">
  <strong>適用任何 Windows 應用程式的語音優先 AI</strong><br/>
  按住一個快捷鍵，就能直接口述、改寫選字內容，或對畫面截圖發問，不必切換視窗也不必自己搬運上下文。
</p>
...
- [Why NeuroPen / 為什麼是 NeuroPen](#why-neuropen--為什麼是-neuropen)
- [Common Use Cases / 常見使用情境](#common-use-cases--常見使用情境)
...
## Why NeuroPen / 為什麼是 NeuroPen

很多 AI 工具的流程是先開網頁、貼文字、補上下文、再把結果貼回原本的 app。NeuroPen 反過來做這件事：你留在原本正在使用的 Windows 應用程式裡，用快捷鍵叫出 AI，處理完就繼續工作。

| 原本常見的阻力 | NeuroPen 怎麼解 |
|------|----------------|
| 要在編輯器、瀏覽器、聊天工具和 AI 分頁間切來切去 | 用全域快捷鍵直接在目前焦點 app 內觸發 |
| 要先複製文字出去，下 prompt，再貼回來 | 可直接改寫選字內容，或把結果安全注入回原本視窗 |
| 小任務也要走完整 AI 流程，很打斷思路 | 口述、翻譯、摘要、潤稿、追問都能用同一套流程完成 |
| 擔心貼錯地方或弄亂剪貼簿 | 注入前驗證焦點，流程結束後還原剪貼簿 |
...
| 情境 | 實際感受 |
|------|----------|
| **隨處語音輸入** | 在 Notion、Word、聊天軟體或瀏覽器輸入框按住 `Alt + 反引號` 說話，放開就轉成文字 |
| **直接改寫選字內容** | 反白一段文字後，用 Quick Action 或直接說「幫我簡短一點」這類指令 |
| **不用切頁就翻譯** | 在任何 app 選字後，透過浮動操作快速轉成目標語言 |
| **對畫面上的內容提問** | 按 `Alt + S` 擷取區域，直接把截圖送進多模態預覽問答流程 |
| **不同 app 有不同規則** | 透過 App Profiles，針對不同應用自動套用語氣、語言、輸出模式與直接貼上設定 |
...
| 能力 | 實際價值 |
|------|----------|
| **全域語音擷取** | 用 `Alt + 反引號` 就能在任何 app 直接說話，不用先切去 AI 視窗 |
| **選字感知分流** | 系統會依你是否有選字，自動判斷是語音輸入、選字處理或助理問答 |
| **Quick Action 浮動操作** | 選字後立即出現可自訂的快捷指令入口 |
| **選取語音指令** | 對著反白文字直接說需求，讓 LLM 幫你改寫、翻譯、摘要或潤稿 |
| **助理對話** | 偵測喚醒詞後切到助理模式，支援串流回覆 |
| **預覽或直接注入** | 可在 `PreviewStream` 先看結果，也可直接回填到目前焦點視窗 |
| **安全注入 + 復原** | 貼回前會驗證焦點視窗，並支援 `Alt + Z` 單步復原 |
| **截圖多模態** | `Alt + S` 擷取畫面後，直接開啟以圖片為上下文的問答流程 |
| **雲端與本地模型** | 支援 OpenAI Whisper API、本地 Whisper、SenseVoice、Moonshine、Ollama、llama.cpp、LM Studio |
| **模型管理** | 可在 app 內安裝、切換、取消或刪除支援的本地 STT 與 Piper TTS 模型 |
| **App Profiles** | 依 app 自動套用語氣、提示詞附加、語言變體、輸出模式與直接貼上規則 |
| **歷史與系統匣流程** | 本地保存歷史紀錄、系統匣常駐、需要時快速叫出 |
...
| 工作流 | 觸發條件 | 你會得到什麼 |
|------|----------|----------------|
| **語音輸入** | 無選字 + `Alt + 反引號` + 無喚醒詞 | 語音先轉文字，必要時再潤飾或翻譯，最後顯示在預覽視窗或直接注入游標位置 |
| **快捷指令** | 有選字 + 點擊 Quick Action | 對選取文字做一次快速處理，結果顯示於預覽視窗 |
| **選取語音指令** | 有選字 + `Alt + 反引號` + 口述指令 | 根據你口述的需求改寫選字內容 |
| **助理對話** | 無選字 + 轉寫中偵測到喚醒詞 | 助理回應顯示於預覽視窗，或依設定直接注入 |
| **截圖提問** | `Alt + S` + 擷取區域 + 輸入問題 | 以截圖為上下文開啟多模態預覽問答流程 |
...
NeuroPen 主打的是「在任何 app 裡直接工作」，所以輸出安全和模型能力一樣重要。文字回填到其他應用前，系統會先走這套固定保護機制：
...
若處理期間焦點改變，系統會取消注入並還原剪貼簿，避免結果貼到錯誤位置。

---

## UI Modules / 介面模組

- **Settings Window / 設定視窗**：集中設定 STT、LLM、TTS、快捷指令、熱鍵、歷史與 App Profiles。
- **Quick Action Icon / 劃詞浮動操作**：選字後出現的一鍵預設指令與自訂輸入入口。
- **Output Preview Window / 輸出預覽視窗**：支援串流輸出、追問、複製/取代與截圖附件預覽。
- **Recording Indicator / 錄音指示器**：用輕量浮窗提示錄音狀態與秒數。
- **History Panel / 歷史紀錄面板**：搜尋、複製與刪除本地歷史輸出。
- **Screenshot Overlay / 截圖區域選取層**：用來擷取畫面並串接截圖問答流程。
- **App Profiles / 應用程式設定檔**：依 app 優先順序自動切換語氣、提示詞附加、語言與輸出行為。
...
| LM Studio | Local | OpenAI-compatible 本地 server，預設 `http://127.0.0.1:1234/v1/chat/completions` |
...
NeuroPen 也可以透過 LM Studio 的 OpenAI-compatible API 連到本地 local server。

1. 先在 LM Studio 對你要用的模型按 `Load Model`
2. 模型載入完成後，打開 `Developer` 分頁並啟用 local server
3. server port 請維持 `1234`，因為 NeuroPen 目前固定連 `http://127.0.0.1:1234/v1/chat/completions`
4. 可先用瀏覽器開 `http://127.0.0.1:1234/v1/models`，或在 PowerShell 執行：

```powershell
Invoke-RestMethod http://127.0.0.1:1234/v1/models
```

5. 在 NeuroPen 的 Settings -> `LLM` 裡選 `LM Studio (Local)`
6. `Model` 欄位填 `/v1/models` 回傳裡的 `id`

補充：

- NeuroPen 預設會連 `http://127.0.0.1:1234/v1/chat/completions`
- LM Studio 在 NeuroPen 中預設不需要 API key；如果你在 local server 開了 auth，也可以填入
- 若已填入 API key，NeuroPen 會把它當成 bearer token 傳給 LM Studio
- 如果 `Invoke-RestMethod` 顯示「目標電腦拒絕連線」，代表 LM Studio local server 還沒真正啟動，光載入模型還不夠
...
| LM Studio 無法連線 | 確認 LM Studio local server 已啟用且 `localhost:1234/v1/chat/completions` 可連線，模型名稱需與目前提供的 model id 一致 |
